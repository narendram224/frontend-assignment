...
export const useFetchWithAbort = (
  ...
  const [saasLabTableData, setSaasLabTableData] = useState<any[]>([]);
  ...
  useEffect(() => {
    if (page && fetchedData) {
      ...
      setSaasLabTableData(result);
    }
  }, [fetchedData, page]);

  return { isLoading, error, saasLabTableData, fetchedData };
};
